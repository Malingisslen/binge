...
describe('buildClearedPayload (DPO hard field-allowlist)', () => {
  ...
  const payload = buildClearedPayload(DELETE, STAMP);
  ...
  it('the freshness stamp maps to the serverTimestamp sentinel (never a delete)', () => {
    expect(payload[TMDB_FIELDS_STAMP]).toBe(STAMP);
  });
