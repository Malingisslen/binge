...
/**
 * Build the field-clearing update payload: every clearable field → the caller's
 * `deleteSentinel` (FieldValue.delete()), plus the freshness stamp → `stamp`
 * (serverTimestamp() sentinel). INVARIANT (DPO must-have, test-locked): the key
 * set is EXACTLY TMDB_DERIVED_FIELDS ∪ {tmdbFieldsRefreshedAt} — never
 * updatedAt, never any user-authored field. This is a fresh object built only
 * from the allowlist; it is NEVER a read-modify-write / merge of the existing
 * doc (that is the #1 way a mixed TMDB+user-authored doc gets corrupted).
 */
export function buildClearedPayload(
  deleteSentinel: unknown,
  stamp: unknown,
): Record<string, unknown> {
  const payload: Record<string, unknown> = {};
  for (const f of TMDB_DERIVED_FIELDS) payload[f] = deleteSentinel;
  payload[TMDB_FIELDS_STAMP] = stamp;
  return payload;
}
