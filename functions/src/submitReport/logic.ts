...
export function validateReportInput(raw: unknown): ValidationResult {
  ...
  if (typeof d.targetOwnerUid !== 'string' || d.targetOwnerUid.length === 0 || d.targetOwnerUid.length > REPORT_ID_MAX)
    return { ok: false, error: 'Ogiltig mål-ägare.' };
  ...
  return {
    ...
    value: {
      ...
      targetId: d.targetId,
      targetOwnerUid: d.targetOwnerUid,
