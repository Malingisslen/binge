...
- **Every review:** the shared review core that ships with the workflow-guards plugin
  (`shared/review-core.md`), then your core card `.claude/agents/binge-security-reviewer.knowledge.md`.
- **Only when the diff touches them:** your chapters, `.claude/agents/binge-security-reviewer.<chapter>.knowledge.md`,
  each tied to path patterns in that config.
- **Never at review time:** the archive, `.claude/agents/binge-security-reviewer.knowledge.archive.md`.
