...
- [#6, STRIKE-FALSE: the 80k cap no longer applies to the split core]
Cap: 80k chars — pay for an addition with a cut, and move what you cut verbatim to the archive file beside this one.
- [#9, STRIKE-DUPLICATE: already in binge-code-reviewer.md "Status-model integrity" and data-model.md]
...
- [#16, sentence 2, STRIKE-FALSE: markers are gone; the ledger is the proof]
So **hash EVERY file a run reads — prod AND test — before and after it**, quote them in the marker, loop `[hash → run → re-hash]` accepting only pre==post==target; re-verify anything Read early against `git show :<path>`.
- [#16, sentence 9, STRIKE-DUPLICATE: lessons-digest 2026-08-25 forbids git checkout -- to undo a foreign edit]
Snapshot/restore with `cp` — but `git hash-object` on a path OUTSIDE the worktree skips the autocrlf clean filter, so a scratchpad copy's sha never matches the index sha; compare in-repo paths only, and prefer `git checkout -- <path>` (index, not HEAD, when the file IS staged) to undo a foreign edit.
- [#17, sentence 3, STRIKE-DUPLICATE: binge-code-reviewer.md "Isolated rigs" carries the junction warning]
**Never junction `node_modules` into the rig** (the old `mklink /J` recipe): any `rm -rf`/`git worktree remove --force` traverses it and EMPTIES the real `C:\binge\node_modules`, and the scratchpad is SHARED per session, so a sibling agent's cleanup of YOUR worktree does it too (2026-08-01 self-inflicted; 2026-08-08 sibling-inflicted, whole repo left with an empty `node_modules` mid-review).
- [#17, sentence 5, STRIKE-MECHANIZED: npm run preflight prints the npm ci remedy]
Remedy: `npm --prefix /c/binge ci --prefer-offline` (~50 s, verify `ls node_modules/.bin/vitest`) — say so, every commit gate is dead until it is back.
- [#17, sentence 6, STRIKE-DUPLICATE: binge-code-reviewer.md names the teardown as shared-guard worktree-cleanup]
Remove the rig with `git worktree remove --force`, or `rm -rf` + `git worktree prune` FROM THE REPO ROOT if it fails `Filename too long`/`Permission denied`.
- [#17, sentence 8, STRIKE-DUPLICATE: lessons-digest 2026-08-01 (LF patch on CRLF)]
**Its files check out CRLF** (`core.autocrlf=true`), so an LF-anchored multi-line mutation patch silently no-matches — and `perl -0pi -e` mis-parses `-0pi` besides.
- [#17, sentence 9, STRIKE-DUPLICATE: lessons-digest 2026-08-05 (assert the mutant before and after)]
Mutate via a script that normalizes `\r\n`, counts occurrences and EXITS NON-ZERO on no-match, and assert the mutant present BEFORE and AFTER the run in the same command.
- [#19, STRIKE-FALSE: markers are gone; the ledger is the proof]
- **Markers:** read CONTENTS, never trust freshness. Append naming your own files (don't overwrite a parallel pass's), name any surface no verdict covers, re-stamp per follow-up. **Diff the SIBLING markers' pinned shas against `git rev-parse :<path>` too** — after a round applies fixes they routinely pin bytes that no longer exist (BIN-645 r2: 1 of 13 pins still current), and the un-covered delta is often the round's ONE behavioural change. **A SERIALIZED reviewer chain inside ONE batch produces the same staleness, and its wording hides it:** BIN-1063 steg 3 batch 1 ran security-r2 (pass, pinned `543db22`/`4362bdf`, saying "both re-pinned after the write-up … the review describes the bytes that will be committed") and only THEN took the test reviewer's r1 finding, which changed `logic.ts`'s exported return type from `HandoverUpdate | null` to the discriminated `HandoverOutcome` — so the security pass named a `^functions/`-gated production file it never read at the staged sha (`870a330`). A re-pin is against that reviewer's OWN round, never against a later sibling's; "both passed these bytes" in YOUR brief is a claim, so `git rev-parse :<path>` against each sibling's LAST pinned sha and diff the blobs (`git diff <oldsha> <newsha>` works on bare blobs) before accepting their ground as settled. A later reviewer's fix un-proves the earlier reviewer, in the direction the commit gate refuses. Say which surfaces their verdicts no longer cover, even when your own verdict passes. Spot-check a marker's EVIDENCE COMMAND, not just its conclusion: BIN-645's security pass closed "consent" on `grep signIn()`, which misses `onClick={signIn}`. A gap it DECLARED may also be closed by bytes newer than its pins. After a SPLIT, sibling OVER-coverage is harmless, but the deferred half owes its own three passes.
- [#22, ARCHIVE: plan-file prose policing]
...
- [#23, ARCHIVE: revert-only, too narrow for the core]
...
- [#26, STRIKE-MECHANIZED: eslint react-hooks/rules-of-hooks runs in lefthook and deploy.yml]
- Hooks run BEFORE any conditional early return — re-verify when a guard lands.
- [#82, ARCHIVE: privacy-policy prose, owned by Legal/DPO (Malin decided 2026-09-19)]
...
- [#110, sentence 4, ARCHIVE: polices comment prose]
**Sweep the WHOLE file, not the flagged line: the fix round has twice shipped a NEW invented route while correcting the old one** (BIN-645 fixed `/sok`→`/search` in seven places, introduced `/rekommendationer/` in the added fixture, and left `?next=/film/1399/` standing in the same module's header).
- [#119, ARCHIVE: comment-vs-code prose policing]
- STALE COMMENT, CORRECT CODE: flag the comment. **Freshly-authored prose gets the same treatment, not a pass for being new** — a brand-new RUNBOOK/ADR line claiming "sweep X releases this automatically" must be checked against the actual predicate, not accepted on plausibility. BIN-879's new RUNBOOK §5f told support that Console-deleting the Auth account makes `orphanedReservations()` release the username within a day (§5e); that function requires BOTH `profileMissing` AND `authMissing`, and the exact scenario the section describes leaves `users/{uid}` intact (the section's own recognition step says so) — so it never fires, and grep confirms no sweep or Auth `onDelete` trigger anywhere reaps a profile with no matching Auth account. A runbook is read live during an incident, so a wrong "this self-heals" line is worse than a stale comment nobody is about to act on in the next five minutes. **A fix to that kind of remedy still needs the SAME check against every OTHER writer keyed on the same id, not just the one predicate that was wrong.** The re-review of that same BIN-879 fix (which correctly named `profileMissing && authMissing` and instructed deleting `users/{uid}` too) still missed `publicProfiles/{uid}` — a SEPARATE top-level doc, not a `users/{uid}` subcollection, so deleting the profile never touches it, and `AuthContext.tsx`'s `syncMyPublicProfile` effect (gated only on `deletionInProgress`, which reads false on the very marker-less device the whole runbook section is about) had already recreated it by the time support opens the case. Grep confirmed zero references to `publicProfiles` anywhere in `functions/src` — no sweep touches it either. When a manual remedy says "delete X", grep every writer touching `X`'s uid for what ELSE it also creates on the same trigger, not just what the original wrong claim was about. **A PAST-TENSE claim about how many COPIES a helper had is a claim about a decision record — check it against `git show HEAD:<file>` and against any ADR the same diff amends.** A round that DELETES a call site routinely miscounts a SHARED helper's second caller as a second copy: BIN-823 r4's `sitemap.ts` header called its use of `collectPersonIds` "en fjärde kopia" when ADR 0005 — amended two files away in the same commit to say "EN delad pipeline, inte två kopior" — exists precisely because there was only ever one. (Its sibling `collectIds` count was right, so verify each name separately, and `git grep <symbol>` at HEAD is the one-command check.) **A diff that REVERSES a documented decision must retire that decision's prose EVERYWHERE it was replicated — grep the OLD ticket id, the DELETED SYMBOL, and any ENV VAR whose scope moved; never the fixer's list of sites, and remember the replication sites are code comments AND `docs/org/adr/*.md` AND the trigger-loaded `.claude/rules/*.md` (BIN-823 r5: `preview.yml` started setting `SELECTION_ALLOW_THIN`; the round corrected the JSDoc + `ci.yml` comment and left ADR 0018 asserting "sätts BARA i `ci.yml` … `deploy.yml`/`preview.yml` kan aldrig råka få den" and `deployment.md` repeating it — one `grep SELECTION_ALLOW_THIN` finds all four). But the VAR grep is NOT the enumerator: r6 fixed all four var sites and left the SAME ADR's Fork E ("Sitemapen kastar … i stället för att falla tillbaka") contradicted by the seed-fallback branch added in the same commit — a decision record authored ALONGSIDE the code it documents is self-contradiction, not staleness, and Fork E never names the var. Grep the BEHAVIOUR word (kastar/faller tillbaka) as well as the symbol, and re-read a new ADR against the diff's last hunk. **A CORRECTION is itself a reversal — re-derive every FORMULA, COMPARATOR and mirrored code comment built on the number you just corrected, starting with the paragraph directly BELOW it:** BIN-823 r7 re-anchored "~10 GB/deploy" and left the raise formula, its comparator and two copies in `selectionManifest.ts` on the retired anchor (~45 % understated against a 25 SEK/mån cap). `git grep` the number — then re-read the SENTENCE it was spliced into (r8 left a dangling `En takhöjning till`). **The same failure mode fires on a PROSE claim, not just a number, and a correction inserted mid-sentence is the tell**: BIN-565's `weeklyDigest/index.ts` spliced a new paragraph between "Titles whose" and "namespaced doc doesn't exist yet…", and left the ORIGINAL absolute — "disappears title by title … digest would go quiet for months" — standing untouched two lines below, still asserting exactly what the new paragraph says is false for a title that left the work set. A correction landing BETWEEN the two halves of one sentence, rather than after the whole block, is itself the signal it was pasted in without reading past the insertion point; check what the sentence says once the splice is read straight through. A round that "took" a prose finding enumerates only the files IT found (BIN-823 r2 left `buildFetch.ts`'s watchdog rationale naming the now-fetch-free `sitemap.ts` as the later phase's biggest un-instrumented fetcher, and counting three `collectIds` copies after the diff deleted one — `git grep collectIds` finds both; a nav claim inside an instrument's own rationale block misaims the next investigation, which is what that block exists to prevent). **A "same pattern as <module>" cross-reference inherits EVERY property of the module it names, and converges only when SPLIT into shared vs explicitly-NOT-shared** — `canonicalSpecials.ts` burned six rounds on one sentence before landing "shares relatedSeries.ts's SHAPE … but not its affordance (tickable) nor its cost"; READ the named module to price each clause, and scope a "nothing is stored here but numbers" claim to the DATA — that same file checks in 22 TMDB episode titles as comments. **JSDoc orphaning runs BOTH ways** — a new doc above an existing function steals it (BIN-616); a new function/test+doc inserted between a block and what it documents leaves that one bare (BIN-641 ×3, incl. a section BANNER left heading a newly inserted `describe`). **A bare `export const` slipped in under an existing block is the cheapest form and a review-round fix is when it happens**: BIN-1063 steg 3 r2 put `HANDOVER_PARTIAL` between `refusalForHandover`'s doc and the function, so a string constant was headed by "Why the caller must NOT proceed …, **or null when it may**" and "so a test can CALL it", both impossible of a `const`, while the function it describes went bare. Read the symbol directly under a doc block in every hunk that ADDS a top-level declaration; the remedy is a MOVE, never new prose. **A comment asserting the fix must not survive beside the code that IS the fix — and the RATIONALE for an affordance must not survive that affordance's deliberate removal, or the file argues against its own code.** BIN-876 r4 kept both halves of the retry-toast justification ("2,5 s räcker inte för att läsa det"; "appens ENDA besked om att biblioteket redan är borta") on the two branches that now ship NO action and are narrated by a persistent screen instead. Same shape one level up: when an ADR condition is superseded mid-build, the ADR records the reversal and `tasks/todo.md` keeps asserting the pre-reversal contract ("both with a retry action", "Support's two strings verbatim") — amend the plan line IN PLACE in the same diff. Grep the claim repo-wide and re-read every ASYMMETRY/never-list the diff falsifies, in the SAME diff. **Verify any comment naming an enforcement or UI SURFACE, and SCOPE a premise to the path it holds for** (BIN-641: "counts the same as from Bibliotek's status menu" — `WatchlistPage` renders no `StatusButton`). **Swapping in a new EXAMPLE never converges — restate the caveat as a PROPERTY**: BIN-641 burned four rounds, each concrete claim wrong in turn, closing as "a caller that does not state intent must not count, whether or not it can currently produce the transition". Justify defence-in-depth as self-consistency, never as a live failure. Read the surface's LABEL and its GUARD; a "no dedupe / always reaches X" claim about a sibling component is a code claim, and a dep-array named in a NEIGHBOURING comment goes stale when the deps change (BIN-645). **An "UNREACHABLE — no caller can produce it" claim is about the callers' RENDER-state inputs while the guard evaluates a LIVE ref** (`buildStatusUpdate`'s `ctx.currentStatus` ← `itemsRef`): any `await` between the caller's decision and the write reopens it. Scope it to "no caller can INTEND it, which is why the branch stays" — an absolute invites deleting a guard that then fires unguarded. Enumerating callers as a CLASS ("CSV import, onboarding, add-all") stays fine — that claims membership, not reachability. **`// eslint-disable-next-line rule -- reason` covers only the LINE AFTER it** (BIN-598: a wrapped reason suppresses nothing and adds "Unused eslint-disable directive"); directive LAST, then `npx eslint`. Read the WHOLE block AND the FILE header (they self-contradict). **An "invisible without X" absolute is usually an unmeasured restatement of "unqueryable without X" — the two differ by a full scan, and the batch's own code is the counterexample.** BIN-1063 steg 2 justified a new `uid` field on `friends`/`friendRequestsSent` with "utan fältet är raden osynlig för varje serversidig städning" in `friends.ts` and a twin in `firestore-rules.test.ts`, while `reclaimOrphanFollows` sweeps `following`/`followers` with an unfiltered `collectionGroup(kind)` scan deriving BOTH endpoints from the path (`d.ref.parent.parent.id`, `d.id`) and no field at all — and that same commit's backfill script finds every fieldless row the identical way. The true half (`documentId()` cannot be filtered against a bare id in a collection-group query, so only a full scan reaches them) was already in the sentence, and the batch's own changelog said it correctly ("utan fältet går de inte att fråga efter"): strike the absolute, keep the filterability clause. Grep the claim in EVERY phrasing before passing a fix — `osynlig`/`hittbar`/`findable` found the two absolutes plus three benign purpose statements, and a round that fixes one copy leaves the other for the push gate. **Steg 3 is the same family one file over, and the cheapest form of it: a batch's OWN two files are each other's counterexample, so check every "the one/only case X exists for" in a TEST comment against the guard's own JSDoc in the sibling production file.** `logic.test.ts:172` justified the noop-vs-delete distinction with "a retried sweep that had already succeeded — which is the one case the guard exists for" while `logic.ts:36-43`, the same batch, names TWO causes for that branch ("either an earlier run already handed it over, or they were only a member") — and conflating noop with delete destroys a live group in the member-only case too, which is the commoner one. A test comment naming a guard's PURPOSE is a claim about production code, so it is this gate's, not only the test reviewer's; that reviewer had read the line twice and passed it as "ambiguous". Strike the clause rather than re-scoping it: the paragraph's real point (the two refusals must be tellable apart) survives its deletion intact. **The same batch's SCHEMA_VERSION bump is the other half of this check, and no test pins it:** `dataExport.coverage.test.ts` lists `schemaVersion` only as a metadata KEY, so `docs/data-export-format.md`'s example block (`"schemaVersion": "2.0"`) went stale in the same commit whose own changelog two screens down said `2.0 → 2.1` — one file contradicting itself. That literal IS maintained (it tracked the 2.0 bump), so it is a correct-in-place case, not a strike. Whenever a diff changes an exported CONSTANT's value, grep the value and the constant name across `docs/**` before passing.
- [#123, STRIKE-DUPLICATE: review-core.md "A test is a claim"]
...
- [#135, STRIKE: unmeasured count "230+" deleted from the core pointer; original line kept here]
...
### 2026-09-23 — BIN-1261: DialogShell's document-capture Escape listener orders by mount, not by topmost dialog
`src/components/ui/DialogShell.tsx` registers its `Escape`/Tab handler with `document.addEventListener('keydown', handler, true)` (capture phase) in a mount-once effect, and calls `e.stopImmediatePropagation()` unconditionally before checking `dismissableRef.current`. Verified empirically in jsdom (not just from the spec text) with a throwaway script: two listeners on the SAME target, same phase, fire in registration order regardless of which was added "capturing" vs "bubbling" relative to each other — jsdom gives capture-phase listeners priority over bubble-phase ones on the target itself (confirmed: a capture listener added AFTER a bubble listener still runs first), but among two capture listeners on the same target, first-registered wins.

Consequence: if a SECOND `DialogShell` instance ever mounts while a first is still open (two dialogs stacked), the OUTER (earlier-mounted, registered-first) dialog's capture listener fires on `Escape` before the INNER (topmost, later-mounted) one, and `stopImmediatePropagation()` prevents the inner one from ever seeing the key — so Escape closes the wrong (outer) dialog, or does nothing if the outer is non-dismissable while the inner would have accepted it. This is backwards from the usual "topmost dialog owns Escape" modal-stacking convention.

Checked reachability before filing: `git grep -n "<DialogShell" -- src` returns exactly two call sites, `ConfirmDialog` and `HandOverGroupDialog`, both used from `GroupSettingsModal.tsx`. `GroupSettingsModal` renders at most one of them at a time in practice — whichever opens first covers the full screen (`fixed inset-0 ... z-[60]`), which blocks the click needed to open the other. So the defect is real but NOT reachable through any call path in this diff or its neighbours; filed as a non-blocking Medium finding, folded into the client chapter rather than the archive-only "unreachable, don't file" pattern, because the NEXT consumer that legitimately stacks two `DialogShell`s (e.g. a confirm-within-a-confirm two-step destructive flow) will hit it silently with a green suite, since nothing in either existing test file exercises two mounted at once.

Fix shape (not built, not requested by BIN-1261's plan or panel condition): a shared module-level LIFO stack of active `DialogShell` instances' dismiss handlers, with only the top of the stack's `Escape` handler acting; or have each instance check "is there a later-mounted instance still active" before handling. Do not build this speculatively — file it if a third consumer appears.

Command used to confirm the jsdom ordering claim before writing this entry (both runs against a bare jsdom, not vitest, to isolate the DOM engine from React/testing-library):
```
node -e "const {JSDOM}=require('jsdom');const dom=new JSDOM('<!doctype html><html><body></body></html>');const document=dom.window.document;const order=[];document.addEventListener('keydown',()=>order.push('bubble-first'),false);document.addEventListener('keydown',()=>order.push('capture-second'),true);document.dispatchEvent(new dom.window.KeyboardEvent('keydown',{key:'Escape',bubbles:true,cancelable:true}));console.log(order);"
```
gives `[ 'capture-second', 'bubble-first' ]` (capture wins over bubble regardless of add order), and the same script with two `true`-capture listeners gives first-added-first, confirming registration order decides ties within the same phase.
