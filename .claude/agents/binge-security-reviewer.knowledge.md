...
- **Mutation-proof any fix whose test could pass for an unrelated reason:** neutralize ONLY the new clause,
  ...
  **A stale module-transform cache FABRICATES results either way — three times now** (latest: BIN-636's
  ...
  mutated LINE in the same command; one clear-then-rerun doesn't earn trust either (BIN-636 failed right after
  the `rm -rf`, passed the next run, no source change — re-run before trusting EITHER verdict).
  **A leftover mutation-harness file INSIDE `src/test/rules/` (not scratchpad) corrupts every LATER run**:
  BIN-624's `__bin624_mutant.test.ts` sat untracked there, globbed into `npm run test:rules`, reporting
  474/10-failed against a real 244/244. `git status --porcelain` the test dir before trusting any count; delete
  a stray `*mutant*`/`*_poc*` file there before believing the suite, not just before finishing.
- **A test title is not coverage** — an `it()` naming "rejects 0 → 1" actually asserted `5`. A mock with FEWER
  ...
  ABSENT, not null). Deny tests whose rule `get()`s the writer's own profile must seed it via
  `withSecurityRulesDisabled`, else they pass vacuously. A test reading its subject back through a SECOND
  validator proves nothing — assert the STORED bytes, or the writer's guard can be deleted and the reader's
  covers for it.
...
- **A type-guard closes the JUNK-VALUE brick, not the MISSING-FIELD brick**, and left-hand
  ...
  path. Enumerate every partial-update writer against the app's smallest real write (a heartbeat, not the
  test's), hunt for a `setDoc(...,{merge:true})` hardcoding the field's INITIAL value (the shape a ratchet
  turns into a hard lockout), and harden with `.get(k,D) is T` on the left. `updateDoc`'s
  `request.resource.data` is the MERGED doc, so a pinned field omitted from the payload inherits and passes.
...
- A new TOP-LEVEL GDPR-cascaded doc needs its own seeded live-emulator assertion in `account-deletion.test.ts`
  — the `KNOWN_USER_SUBCOLLECTIONS` loop misses it, and a mocked test proves wiring, not live erasure. That
  const + the three-way set-equality guard (rules paths == const == helper `collection()` reads == `keyof
...
- CG export queries (`likes`/`comments`/`reactions`) need (a) a `uid` FIELD per doc — doc-id alone is
  ...
  { allow read: if isSignedIn() && resource.data.uid == request.auth.uid; }`); nested per-doc rules do NOT
  cover CG.
- **Collector change needed?** New subcollection → yes; new FIELD on covered doc → no; new doc-ID SCHEME in a
  covered subcollection → no (`getDocs` is doc-id-agnostic). Tab-local ephemeral state isn't a collection.
...
- **A read rule conditioned on more than plain membership (reciprocity `exists()`, roles, `isAdmin()`) can break
  ...
  carve-out (own doc-id discloses no third party); `list` keeps the strict gate. Suites that always seed the
  actor's own doc MASK this (BIN-184).
- **Consent honesty:** `termsAcceptedAt`/`termsVersion`/`ageConfirmedAt` write only in `ensureUserProfile`'s
  ...
  Never backfill phantom consent; enumerate ALL orderings on a consent-bearing write race (BIN-535). Privacy
  §8's `binge:` coverage is deliberately partial (LEK-exempt) — growing it is Info, not a re-file trigger.
...
- **Invalidate a per-account cache/ref/mirror on the OWNER IDENTITY, never on the owned VALUE.** A value-keyed
  ...
  whose dep array holds only payload; a faithful refactor of a wrong key inherits the bug. Verify four ways:
  the key is the RIGHT identifier (`uid` from `onAuthStateChanged`, not the lagging `user.uid`) and CURRENT (a
  LAYOUT effect); SIGN-OUT resets A→null; SIBLINGS carry the class; an identity-checked rollback
  (`if (ref.current === next) ref.current = prev`) stops a late reject.
- **When a sign-out handler clears per-session refs, audit the reset SET — what it clears AND what it keeps.**
  A per-login retry latch MUST clear (BIN-617: `visibilityRetriedFor.current = null` in the sign-out reset,
...
- **Per-uid listener state that resets only inside `if (!uid)` leaks across a truthy→truthy uid switch**
  ...
  **The guard doesn't travel with the ref — check every call site sharing it, not just effects with one
  already.** BIN-598's shared `findItem()` read the same `itemsRef` but skipped the `itemsUidRef` check. Split
  in two: WRITE PATH closure-bound (safe — `doc(db,'users',uid,...)` uses the caller's own account) vs WRITE
  DECISION read off the shared ref (the leak) — an in-flight call from A could have `findItem` return B's cached
  item for a shared tmdbId after a same-device switch, firing `shouldStampVisibility` on FOREIGN data while
  writing A's own doc. **FIXED + mutation-verified (2026-08-04):** `findItem` returns `undefined` when
  `itemsUidRef.current !== uid`. Residual non-blocking: that also reads as "no override" in `addItem`'s
  timestamp guards — reopen only if a UI ships per-item visibility writes.
- **A deny-list redaction on a doc with no `hasOnly` write-whitelist eventually leaks a newly-accreted field**
  ...
  proven by a test toggling the source mid-test; (4) `isSignedIn() &&` before the friend-branch `exists()`;
  (5) **audit EVERY client call site reading a FOREIGN `users/{uid}` BEFORE tightening** — rules-only is an
  outage.
- **Retiring a field you can't drop from `hasOnly`:** the `update` guard needs the three-way OR — absent /
  ...
  on a legacy doc. Find every place the OLD shape is read and re-passed; prefer a whitelist-copy helper over
  spread+delete (a removed TYPE is not a runtime strip).
- **A shared helper collapsing a three-way distinction into a two-way return re-opens the ghost-vs-private
  ...
  vanished from the follow list). Row presence/absence is itself a leak; one anonymous fallback row for both
  is the IMPROVEMENT. Check `error.code === 'permission-denied'`; no error means "not opted in".
...
- Symmetric mirror writes must NEVER be gated by `isOwner(uid) || isOwner(targetUid)` alone — the second
  ...
  `exists()` when the proof is deleted in the SAME batch; `existsAfter()` when proof and mirror are created
  together (BIN-20). Rules gate KEYS, not field COMPLETENESS — a client omitting `followedAt` reads as "old"
  in reclaim sweeps. A member-create rule's `get(group)` DOES see a doc written earlier in the batch.
- **A compensating/rollback write needs its own TOCTOU check — UNLESS the doc is provably unreachable by
  ...
  SAME call before its id/token left the function (BIN-555 createGroup: no invite token handed out yet, no
  second writer possible) — delete-the-whole-doc, not `arrayRemove` the lone member, is also the right SHAPE
  there: no "other member" to preserve, and `arrayRemove` would leave the ownerless orphan the rollback exists
  to remove while hiding it from the owner's own `array-contains` query.
...
- A top-level collection with NO rules block is default-denied — safe ONLY because this rules file has no
  `match /{document=**}` catch-all; re-verify on each approval (`*Budget`, `*State`, `sweepState` families).
  Same for a MUTATE GATE: `sweepState/tmdbFieldsSweep.mutateEnabled` is Console-only — dry-run-by-default is a
  data field, never a deploy flag.
- Public-read no-PII rollups (`streamingLeaving`, `recaps/**`, `priceHistory`, `streamingOffers`,
  ...
  enclosing match block; single-segment wildcards cover new doc-id schemes.
- External API ids used as URL path + lookup key get a strict format guard (Wikidata `/^Q\d+$/` +
  `encodeURIComponent()`); external maps consume only strictly-parsed fields (`(movie|tv)/<positive int>`);
  on fetch failure return null, never overwrite the prior doc.
- **Post-sign-in return path (BIN-645, archive corpus): the parse is never the residual — CARRIER + WRITE SITE
  are.** Parse guard: first char `/`, second neither `/` nor `\`, plus a control-char scan. A `?next=` QUERY
  both LEAVES the origin (Firebase's popup copies the URL to the Google-hosted auth handler) and is
  ATTACKER-SUPPLIED (`?invite=` auto-joins on mount); `sessionStorage` fixes both, but the WRITE SITE
  (`location.pathname + location.search`) can still express a query — hence a `RETURN_QUERY_KEYS` allowlist
  on BOTH legs. The PATH half can't be allowlisted (it IS the feature); "onboarding OUTRANKS the stored path"
...
- **Consume-on-read makes the consuming effect NON-IDEMPOTENT** — a second run (StrictMode, extra identity
  ...
  auth resolves, but "resolved" needs its OWN two predicates: AuthContext keeps `uid` and nulls `user` on a
  profile-read reject, so gating on `user` alone never fires — gate on `uid` + the loading flag, `user != null`
  only on onboarding. A loading FLAG + nullable payload must distinguish "resolved-absent" from "not yet".
- Third-party `href`: reject unless `startsWith('https://')` (`safeHref`, `rel=noopener`). Prose renders as
  plain-text React children, never `dangerouslySetInnerHTML`; JSON-LD needs `.replace(/</g,'\\u003c')`. Recap
  text validates BOTH sides (read-side authoritative: 4000-char cap, HTML/URL/markdown guards); free text with
  no `hasOnly` cap gets length-capped in the CONSUMING function before an FCM body.
...
- Rules can't count sibling writes in a writeBatch — one batch carried N report-creates past a per-batch
  throttle. Fix: server-authoritative `onCall`, per-uid cooldown in `runTransaction`, `reporterUid` from auth.
...
- **Test a gitignore secret pattern with `git check-ignore`** against the RUNBOOK's literal filename AND the
  tool's DEFAULT download name: `*-recaps-writer*.json` misses `recaps-writer.json`; GCP's `{project}-{hash}.json`
...
- Sealing a client-writable counter behind a callable closes FORGERY/write-SHAPE, NOT COST or DOC-COUNT.
  ...
  `titleRatingsRateLimit/{uid}` per-caller, `recapCoverageGaps` none.
- Identify the ACTUAL enforcement vs the design-intent justification — vendor spend is bounded by
  `reserveMotnSlot`'s transaction, not schedule cadence (a wrong comment is a Low). Never refund a reservation
  the vendor already counted.
- **"Claim in a transaction, work outside, release on failure" has a hard-timeout hole.** GCP force-kills a
  timed-out function with no `finally`, so a kill between claim and release sticks the flag for the cycle and
  the alert never fires — bound the window under the platform timeout, or give the claim a LEASE timestamp.
  Transactions retry the callback, so `.add()` duplicates; use idempotent `.doc(id).set()`.
...
- **Admin SDK bypasses `firestore.rules` entirely — `hasOnly` offers ZERO protection against what a sweep
  writes.** The backstop is pure logic: a FRESH payload from a fixed allowlist (never spreading the stored
...
- **collectionGroup matches by LEAF collection id regardless of parent path** — grep other writers/readers of
  ...
  `status` — prefer `isUserWatchlistDocPath`. Append each page BEFORE the `size < PAGE_SIZE` break.
- uid must come from the doc PATH (`d.ref.parent.parent?.id`), never client content — re-verify each adopter.
- Bare-tmdbId keying collides movie and TV titles (independent TMDB namespaces): grouping keys, state doc
  ...
  keying bug → grep every OTHER collection keyed on the same bare id (BIN-523 failed on this). Legacy-fallback
  reads/DELETEs gate on the legacy doc's own `mediaType`; a dedup swap onto a fresh empty collection double-
  pushes once.
- A doc-id namespacing migration on a doc holding a MULTI-WRITER map needs a VALUE-level merge, never a
  ...
  namespaces — pin it in a test). A doc id is a trust boundary only if it appears in the allow-expression: BIN-624
  added a `canonicalSwipeDocId(tmdbId)` regex guard to `swipes`' `create` branch only (update stays open for
  BIN-608's merge) — FORM-bound, not VOLUME-bound (`movie_1`…`movie_999999` still creatable); don't read it as
  closing the junk-doc-cost class (ADR 0015).
- **A LOOSE doc-id parser makes ALIASES collide in a last-write-wins Map** (`'movie_042'` took genuine
  `movie_42`'s slot, BIN-618): require the parser be the writer's STRICT inverse. A client-strict /
  server-permissive divergence reached only via an already-unvalidated fallback field is DOMINATED: Info, not a
  finding. A "fall back to the nearest earlier X" loop needs a monotonic-DECREASING cursor that never overshoots.
...
- Order is direction-dependent and load-bearing — decide it. Name a TARGETED command (`--only
  ...
  depends on them (a CG `where('uid','==',uid)` export query deployed before its rules makes GDPR export AND
  deletion throw for everyone); AFTER hosting when a new constraint would deny writes from the OLD still-running
  client (BIN-540's ratchet vs a live tab).
- **`deploy.yml`'s rules/functions drift guard runs FIRST with no `continue-on-error`, so a commit touching
  ...
  PARENT commit for BOTH surfaces: the honest baseline for a revert analysis.
...
- **An attestation is a load-bearing claim — verify it like code**, whether it sits in a comment, a JSDoc, a
  `tasks/todo.md` "condition closed" note, or your own last marker. A false safety comment is WORSE than none
  because it says the audit happened (BIN-523's "the scan filters mediaType" was false exactly where it
  mattered). **A REDESIGN orphans comments:** after `?next=` became sessionStorage the login page still
  credited `safeNextPath` with stopping it, inviting the next author to re-add the param believing it
  guarded — grep the REMOVED mechanism's name across the diff's files. **A "line-endings only" claim needs BYTE
  ACCOUNTING, not an eyeballed `--ignore-cr-at-eol` diff:** that flag ignores a TRAILING CR but not a LONE one,
  which is content — so a stray CR surfaces as a phantom edited line. Reconcile counts (5568→5436 B = 133 CRs
  dropped, one becoming an LF) before accepting it.
- **Challenge a dispatching prompt's premise, and scope a RE-review by SHA.** "NEVER had a security review",
  "everything else is as you last saw it", "no marker covers these files" and "only file X moved" have each been
  false. ALWAYS diff every staged sha against what the marker pinned: equality is the spot-check, inequality is
  the real scope, the brief's file list is a hint, never the boundary — check `git diff --cached --stat` too.
  **But sha equality proves the BYTES held, not that their CLAIMS do** — a byte-identical module's doc comment
  is falsifiable by a change elsewhere in the diff (`nextPath.ts` ↔ `QuickAddButton.tsx`). An unchanged file
  the diff depends on is exempt from re-reading, never from re-derivation.
- **When a diff's ticket matches a gap NAMED in a prior entry, re-grep to confirm FULL closure** — a targeted
  fix routinely leaves an identical-pattern sibling unfixed. A ticket AC promising "a new test proves X" with no
  test file in the diff is itself the finding.
- **When a plan/ADR's Consequences says a field "will be added to `hasOnly()`", verify `firestore.rules` was
  actually touched.** An Admin-SDK-only v1 can defer it, but flag the commitment — the moment a client write
  path adds that key, `hasOnly` rejects the ENTIRE write. Flag deviations from the project's own binding
  conditions even when cost-labeled.
- **Shared-checkout hazard (markers are gone, but a live sibling session isn't).** A live sibling's mutation
  loop can land INSIDE your window — 3× now, once with the `Read` anomaly and the `MM` on TWO DIFFERENT files,
  so a clean `git status` on one file is NOT proof — the loop may have moved past it. `Read` serves the
  WORKTREE: hash against `git rev-parse :<path>` when something looks wrong. While a sibling's loop is live
  anywhere, distrust one result on any file: re-`Read` + re-run `--no-cache` twice, identical results
  required — disagreement with no edit of your own IS the signal. Settle the tree, re-verify once more.
