...
- [0020](0020-username-release-and-partial-cascade-signal.md) — BIN-875/876: the orphaned username reservation is recovered by uid-query (not the marker, not the profile doc) and released by the reaper too; a mid-cascade failure gets its own in-memory signal and its own honest message — no persisted cursor. Three scope questions escalated to Malin
- [0021](0021-bin813-preflight-hardened-not-rebuilt.md) — BIN-813: the described bug is unreachable after BIN-816, but the pre-check still never reads the marker — harden it at the source in AuthContext and close asks (1)/(2)/(4) as already shipped; never a second memory mechanism, never a shortcut past the recent-login gate (Malin)
- [0022](0022-cross-device-deletion-gap-accepted.md) — BIN-879: the cross-device aborted-deletion gap is ACCEPTED (only the account holder can trigger it; the 25 collections are already erased) with #6 DPO dissenting on the record; the re-stamped `termsAcceptedAt`/`ageConfirmedAt` consent record is NOT accepted and is filed separately — a fix in `userDocWrite.ts` would be a no-op (Malin)
