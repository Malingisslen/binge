    ...
    queryFn: async (): Promise<Map<string, Map<number, MemberProgress>>> => {
      ...
      try {
        ...
        await Promise.all(watchlistSnap.docs.map(async itemDoc => {
          // BIN-560 Phase 5 fold-fix: the inner map keys by bare tmdbId, so a movie
          // and a same-numbered TV show would collide on one key. Group progress is
          // TV-ONLY (only syncProgressToGroups writes it, always for TV; a movie's
          // watchlist doc never has a progress subcollection) — so source-filter to TV
          // BEFORE keying, mirroring diary.ts's showById fix. This drops movie docs
          // (which carry no progress anyway) and removes the collision at its root.
          if (itemDoc.data().mediaType !== 'tv') return;
          const tmdbId = resolveTmdbId(itemDoc.data().tmdbId as number | string | null | undefined, itemDoc.id);
          if (!Number.isFinite(tmdbId)) return;
          // Build the progress subpath from the ACTUAL (now namespaced, tv_123) doc id.
          // BIN-560 Phase 5: the WRITE half (groups.ts syncProgressToGroups /
          // setGroupMemberProgress) now also namespaces via mediaTypeDocId, so read and
          // write agree on the tv_123 shape.
